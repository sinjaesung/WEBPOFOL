...
class App {
    ...
    _setupShot() {
        ...
        window.addEventListener("click", event => {
            ...
            const pt = { 
                x: ( event.clientX / width ) * 2 - 1,//-1~1(화면좌측일수록-1,화면우측일수록1)
                y: - ( event.clientY / height ) * 2 + 1//1~-1(화면아래일수록 -1->위일수록1)
            };
            console.log("x,y 이차원 정규화좌표:",pt);
            raycaster.setFromCamera(pt, this._camera);

            const tmpPos = new THREE.Vector3();
            tmpPos.copy(raycaster.ray.origin);
            console.log("raycaster and ray origin:",raycaster.ray.origin);
            console.log("and camera좌표:",this._camera.position)
            const pos = {x: tmpPos.x, y: tmpPos.y, z: tmpPos.z};
            const radius = 0.25;
            const quat = {x: 0, y: 0, z: 0, w: 1};
            const mass = 1;
            console.log("raycaster tmppoos copy:",tmpPos);
            ...
            console.log("ball positionsss:",pos);
            ...
            console.log("tmpoPos ray direction:>>>>>>>>>>>>>>>>>>>>>>>>>>>>",raycaster.ray.direction,tmpPos);
    ...
    _setupAmmo() {
        Ammo().then(() => {
            console.log("ammo??:",Ammo,new Ammo.btDbvtBroadphase(),new Ammo.btDefaultCollisionConfiguration())
            const overlappingPairCache = new Ammo.btDbvtBroadphase();
            const collisionConfiguration  = new Ammo.btDefaultCollisionConfiguration();
            const dispatcher = new Ammo.btCollisionDispatcher(collisionConfiguration);
            const solver = new Ammo.btSequentialImpulseConstraintSolver();
            console.log('dispacher,solver:',dispatcher,solver);
    ...
    _createDomino() {
        const controlPoints = [
            ...
            [-2., 0., 0.],
            [0., 0., 0.],
        ];//화면의 왼쪽도미도 시작점에서 끝점까지의 나선형으로 돌고들어가는 베이지어커브모양 controlpoints추상화
    ...
    update(time) {
        ...
        //console.log("deltaTimess:",deltaTime);
